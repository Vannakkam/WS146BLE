...
Target board     : Waveshare ESP32-S3-Touch-LCD-1.46
  ...
  - PWR + BOOT buttons
  - CST816S capacitive touch (same I2C bus, TP_INT=GPIO4, TP_RST=EXIO1)
...
H4  [Touch enabled]   Added CST816S driver (init, read, IRQ hookup on
...
#define TP_INT_PIN        4
#define CST816S_ADDR      0x15      // typical for Waveshare 1.46" capacitive touch
...
// =============================================================================
// CST816S touch driver  (H4 -- previously unused)
//   Minimal: read 0x02..0x05 (X_H, X_L, Y_H, Y_L) on IRQ.
//   Touch IC address may vary; if 0x15 doesn't ACK, no touch -- LCD still works.
// =============================================================================
bool touchInit() {
    pinMode(TP_INT_PIN, INPUT_PULLUP);
    Wire.beginTransmission(CST816S_ADDR);
    if (Wire.endTransmission() != 0) {
        Serial.println("[TOUCH] CST816S not found at 0x15 -- touch disabled");
        return false;
    }
    // Software reset (reg 0xE4 = 0x00 on some CST816 variants)
    i2c_write_reg(CST816S_ADDR, 0xE4, 0x00);
    delay(20);
    // Motion mask: enable motion + motion-up events
    i2c_write_reg(CST816S_ADDR, 0xEC, 0x01);
    // IRQ polarity: active-low
    i2c_write_reg(CST816S_ADDR, 0xFA, 0x00);
    i2c_write_reg(CST816S_ADDR, 0xFB, 0x00);
    Serial.println("[TOUCH] CST816S OK");
    return true;
}

void IRAM_ATTR onTouchIsr() {
    // Just flag; do the I2C read from lvglTouchRead() in the LVGL thread.
    // The IRQ is level-low so it stays asserted until we read the status reg.
    touchHasEvent = true;
}
...
bool touchRead(int16_t* x, int16_t* y, bool* pressed) {
    ...
    touchHasEvent = false;
    uint8_t buf[6];
    if (!i2c_read_regs(CST816S_ADDR, 0x02, buf, 6)) {
        return false;
    }
    // buf[0] = gesture id, buf[1] = num points (0..1)
    uint8_t nPoints = buf[1] & 0x0F;
    if (nPoints > 0) {
        touchX = ((buf[2] & 0x0F) << 8) | buf[3];
        touchY = ((buf[4] & 0x0F) << 8) | buf[5];
        touchPressed = true;
        backlightNoteTouchActivity();   // B6: any real touch wakes/extends backlight
    } else {
        touchPressed = false;
    }
    *pressed = touchPressed;
...
void imuTask(void*) {
    ...
    for (;;) {
        ...
        // C2: CSV ring fills whenever the task runs (no client gate)
        if (csvBuffer && csvWriteIdx < csvMaxBytes - 100) {
